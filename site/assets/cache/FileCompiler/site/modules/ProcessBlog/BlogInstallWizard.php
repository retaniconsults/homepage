...
class BlogInstallWizard extends ProcessBlog {
	...
	// the type of blog style selected by \ProcessWire\user(1-4)
	private $blogStyle;
	...
	*/
	public function verifyInstall($form) {
		...
		// 1. ###### First we check if our parent BLOG \ProcessWire\page(S), role [blog-author], fields and templates already exist.
		...
		*/
		...
		if($installWizardBtn && $installWizardBtn == 'Run install wizard') {// was the right button pressed
			...
			elseif ($this->blogStyle == 3 || $this->blogStyle == 4) {
						...
						}// end foreach checks of all parent \ProcessWire\pages(i.e. posts, categories, tags,comments [if applicable], widgets, authors, archives and settings)
	...
	*/
	private function inContextFieldSettings() {
		...
		// But we need to first create the repeater \ProcessWire\page(the 'for-field-id' page)
	...
	*/
	private function createPages() {
		...
		// 7. ###### Create needed parent pages and some example child \ProcessWire\pages(total pages vary depending on blogStyle selected) ######
		...
		// create the remaining parent \ProcessWire\pages(i.e. minus 'blog' which we created before)
		...
		// create the child pages: // $k=for parentsID [where applicable] => $v[0]=title; $v[1]=template; $v[2]=parent \ProcessWire\page(OBJECT)
	...
	*/
	private function extraPageSettings() {
		...
		$f->derefAsPage = 0;// multiple \ProcessWire\pages(PageArray) - i.e. \ProcessWire\FieldtypePage::derefAsPageArray;
		...
		if ($this->commentsUse == 1) {
					...
					$f->derefAsPage = 1;// single \ProcessWire\page(PageArray) or boolean false when none selected - i.e. \ProcessWire\FieldtypePage::derefAsPageOrFalse
	...
	*/
	private function createTemplateFiles() {
		...
		if ($this->templateFilesInstall == 1) {
					...
					$sourcepath = dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/ProcessBlog/BlogInstallWizard.php') . '/template-files/';// source of the template files to copy over [this is 'ProcessBlog/template-files/']
		...
		elseif ($this->templateFilesInstall == 2) {
					...
					$sourcepath = dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/ProcessBlog/BlogInstallWizard.php') . '/template-files/';
					$destinationpath = $config->paths->templates;// destination: '/site/templates/'
					...
					if($this->demoBlogScriptsInstall) {
						...
						$sourcepath = dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/ProcessBlog/BlogInstallWizard.php') . '/scripts/';
					...
					if($this->demoBlogCSSInstall) {
						...
						$sourcepath = dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/ProcessBlog/BlogInstallWizard.php') . '/css/';
						...
						$sourcepath = dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/ProcessBlog/BlogInstallWizard.php') . '/css/images/';
