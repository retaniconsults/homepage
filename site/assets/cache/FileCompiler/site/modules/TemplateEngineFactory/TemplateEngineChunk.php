 ...
 */
class TemplateEngineChunk extends \ProcessWire\WireData
{
    ...
    public function setChunkFile($chunk_file)
    {
        if (!is_file($this->getChunkPath($chunk_file))) {
            throw new \ProcessWire\WireException("Chunk file does not exist: '{$chunk_file}'");
        }
    ...
    public function setTemplateFile($template_file)
    {
        ...
        if ($this->view === null) {
            throw new \ProcessWire\WireException("View for chunk {$this->chunk_file} does not exist");
        }
    ...
    {
        $chunk = new \ProcessWire\TemplateFile(\ProcessWire\wire('files')->compile($this->getChunkPath($this->chunk_file),array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
