 ...
 *
 */
abstract class TemplateEngine extends \ProcessWire\Wire
{
    ...
    public static function getModuleConfigInputfields(array $data)
    {
        ...
        $f->label = \ProcessWire\__('Path to templates');
        $f->description = \ProcessWire\__('Relative path from the site directory where template files are stored. E.g. "templates/views/" resolves to "/site/templates/views/"');
        $f->value = $data['templates_path'];
        $f->required = 1;
        $wrapper->append($f);

        $f = $modules->get('InputfieldText');
        $f->label = \ProcessWire\__('Template files suffix');
        $f->description = \ProcessWire\__('File extension of template files');
        ...
        $f->label = \ProcessWire\__('Global template file');
        $f->description = \ProcessWire\__('Filename of a template file that is used as main template behind the API variable');
