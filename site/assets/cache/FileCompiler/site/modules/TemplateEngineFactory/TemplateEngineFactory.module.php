<?php
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TemplateEngineFactory' . '/TemplateEngineNull.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TemplateEngineFactory' . '/TemplateEngineChunk.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
 ...
 * - Adds hooks for rendering output over template engine and clear \ProcessWire\cache(s) when modifying pages
 *
 * @author Stefan Wanzenried <stefan.wanzenried@gmail.com>
 * @license http://www.gnu.org/licenses/gpl-2.0.txt GNU General Public License, version 2
 * @version 1.1.1
 */
class TemplateEngineFactory extends \ProcessWire\WireData implements \ProcessWire\Module, \ProcessWire\ConfigurableModule
{
     ...
     * If a template file is found, adds hook after \ProcessWire\Page::render to output markup from engine.
    ...
    public function ready()
    {
        ...
        if (in_array('TemplateEngineCache', \ProcessWire\wireClassImplements($engine))) {
    ...
    /**
     * Method executed after \ProcessWire\Page::render()
     * If we are in the admin or the factory is not active, return early
     *
     * @param HookEvent $event
     */
    public function hookRender(\ProcessWire\HookEvent $event)
    {
        $page = $event->object;
     ...
     * @param HookEvent $event
     */
    public function hookClearCache(\ProcessWire\HookEvent $event)
    {
    ...
    public function getInstanceById($class, $filename = '', $as_api_var = false)
    {
        ...
        if (!in_array($class, array_keys($installed))) {
            throw new \ProcessWire\WireException("TemplateEngine with class {$class} is currently not installed");
        }
        /** @var TemplateEngine $engine */
    ...
    public static function getModuleConfigInputfields(array $data)
    {
        ...
        $f->label = \ProcessWire\__('Template Engine');
        $f->description = \ProcessWire\__('Select the template engine which is used to render your templates.');
        $f->notes = \ProcessWire\__('More config options available in the selected TemplateEngine module.');
        $f->value = $data['engine'];
        $f->name = 'engine';
        ...
        $f->label = \ProcessWire\__('API variable to interact with the view');
        $f->description = \ProcessWire\__('Enter the name of the API variable with which you can interact with the current active template');
        $f->name = 'api_var';
        ...
        /** @var InputfieldText $f */
        $f = $modules->get('InputfieldText');
        $f->label = \ProcessWire\__('API variable for the TemplateEngineFactory module');
        $f->description = \ProcessWire\__('Enter the name of the API variable that returns a singleton of this module, to load chunks and views');
