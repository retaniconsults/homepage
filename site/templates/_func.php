...
function renderNav(PageArray $items) {
	...
	if($out) $out = "<ul class='navbar-nav main-menu'>$out</ul>\n";
...
function renderNavTree($items, $maxDepth = 0, $fieldNames = '', $class = 'nav') {
	...
	foreach($items as $item) {
		...
		// if current item is the same as the page being viewed, add a "current" class to it
		$out .= $item->id == wire('page')->id ? "<li class='current nav-item'>" : "<li class='nav-item'>";

		// markup for the link
		$out .= "<a class='nav-link' href='$item->url'>$item->title</a>";
		...
		if($item->hasChildren() && $maxDepth) {
			if($class === 'nav') $class = 'nav-item dropdown';
